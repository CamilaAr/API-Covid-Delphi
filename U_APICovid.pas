...
//Ordena pelas opções disponiveis
...
begin
  try
    ...
    begin
      ...
      // Verificar se a resposta é válida (não nula e em formato JSON)
    ...
    begin
      // Tratar códigos de status que indicam erro
      ShowMessage('Erro ao acessar a API. Código: ' + HttpResponse.StatusCode.ToString);
    end;
  except
    on E: ENetHTTPClientException do
      ShowMessage('Falha na comunicação com a API: ' + E.Message);
    on E: Exception do
...
begin
  ...
  try
    ...
    begin
      ShowMessage('Erro ao parsear o JSON. Formato inválido.');
      Exit;
    end;

    // Verificar se o campo "data" existe e é um array
    CountriesArray := JsonData.GetValue('data') as TJSONArray;
    if not Assigned(CountriesArray) then
    begin
      ShowMessage('Formato de dados inválido. Campo "data" não encontrado.');
      Exit;
    end;
